...
def distorted_bounding_box_crop(image,
    ...
    Args:
        ...
        aspect_ratio_range: An optional list of `floats`. The cropped area of the
            image must have an aspect ratio = width / height within this range.裁剪后长宽比
        area_range: An optional list of `floats`. The cropped area of the image
            must contain a fraction of the supplied image within in this range.最少占整张图像的多少
    ...
    with tf.name_scope(scope, 'distorted_bounding_box_crop', [image, bboxes]):
        ...
        #随机截取图像信息：bbox_begin, bbox_size用于裁剪图像  distort_bbox用于显示裁剪的边框
...
def preprocess_for_train(image, labels, bboxes,
    ...
    with tf.name_scope(scope, 'ssd_preprocessing_train', [image, labels, bboxes]):
        ...
        dst_image = image
        #随机截取图像信息MIN_OBJECT_COVERED=0.25 与任一目标框重叠区域大于0.25 aspect_ratio_range=（0.6，1.67）
